...
class CNFSafetyLivenessGenerator(AstGenerator):
    ...
    def generate(self) -> CNFFormula:
        v = fol.VariableGenerator(variable_names=self.variable_names)
        f = fol.FunctorGenerator(variable_gen=v, arities=self.functor_arity,
                                 functor_names=self.functor_names,
                                 max_recursion_depth=self.functor_recursion_depth)
        p = fol.SafetyLivenessPredicateGenerator(variable_gen=v, arities=self.predicate_arities,
                                                 predicate_names=self.predicate_names, functor_gen=f)
        a = fol.AtomGenerator(variable_name_gen=v, connectives=self.atom_connectives, predicate_gen=p)
        l = fol.LiteralGenerator(negation_chance=self.literal_negation_chance, atom_gen=a)
        ...
        for solution in solver.solve_in_random_order():
            ...
            for clause_len, amount_of_clauses in solution.items():
                c = fol.CNFClauseGenerator(clause_lengths={clause_len}, literal_gen=l)
                clause_gens[c] = amount_of_clauses
            F = fol.CNFFormulaGenerator(clause_gens=clause_gens)
            yield F.generate()
