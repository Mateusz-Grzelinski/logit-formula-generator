...
from src.ast.first_order_logic import CNFFormula, CNFClause, Literal, Predicate, Functor, Variable, Atom
from src.ast.first_order_logic._fol_visitor import FOLVisitor
from src.ast.first_order_logic.conjunctive_normal_form.cnf_formula_info import CNFFormulaInfo


class MathSense:
    @staticmethod
    def hash_variable(scope: CNFClause, variable: Variable) -> int:
    ...
    @staticmethod
    def hash_atom(scope: CNFClause, atom: Atom) -> int:
        item_hashes = []
    ...
    @staticmethod
    def hash_literal(scope: CNFClause, literal: Literal) -> int:
        return hash((literal.is_negated, MathSense.hash_atom(scope, literal.atom)))

    @staticmethod
    def hash_clause(clause: CNFClause) -> int:
        return hash((MathSense.hash_literal(clause, i) for i in clause._items))


class CNFFormulaVisitor(FOLVisitor):
    def __init__(self):
