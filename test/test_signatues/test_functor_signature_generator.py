...
class TestFunctorSignatureGenerator:
    def test_functor_max_recursion_depth_0_arity_empty(self):
        ...
        fg = FunctorSignatureGenerator(arities={}, max_recursion_depth=0, random=True)
        functors = list(fg.generate())
        assert functors == []

    def test_functor_max_recursion_depth_0_arity_0(self):
        fg = FunctorSignatureGenerator(arities={0}, max_recursion_depth=0, random=True)
        functors = list(fg.generate())
    ...
    def test_functor_max_recursion_depth_0_arity_1(self):
        fg = FunctorSignatureGenerator(arities={1}, max_recursion_depth=0, random=True)
        functors = list(fg.generate())
        assert Functor(name=fg.functor_name, items=[Variable(name=fg.variable_name)]) in functors
        assert len(functors) == 1

    def test_functor_max_recursion_depth_0_arity_2(self):
        fg = FunctorSignatureGenerator(arities={2}, max_recursion_depth=0, random=True)
        functors = list(fg.generate())
        assert Functor(name=fg.functor_name,
                       items=[Variable(name=fg.variable_name), Variable(name=fg.variable_name)]) in functors
        assert len(functors) == 1
    ...
    def test_change_in_variable_name_affects_only_one_variable(self):
        ...
        assert functor._items[0].name == 'V2' and functor._items[1].name == fg.variable_name

    def test_functor_recursion_depth_0_multiple_arities(self):
        fg = FunctorSignatureGenerator(arities={0, 1, 2}, max_recursion_depth=0, random=True)
        functors = list(fg.generate())
        assert Functor(name=fg.functor_name, items=[]) in functors
        assert Functor(name=fg.functor_name, items=[Variable(name=fg.variable_name)]) in functors
        assert Functor(name=fg.functor_name,
    ...
    def test_functor_recursion_depth_1_arity_0(self):
        # todo probably should raise some exception in this case....
        fg = FunctorSignatureGenerator(arities={}, max_recursion_depth=1, random=True)
        functors = list(fg.generate())
        assert functors == []

    def test_functor_recursion_depth_1_arity_1(self):
        # todo probably should raise some exception in this case....
        fg = FunctorSignatureGenerator(arities={1}, max_recursion_depth=1, random=True)
        functors = list(fg.generate())
        func_non_rec = Functor(name=fg.functor_name, items=[Variable(name=fg.variable_name)])
    ...
    def test_functor_recursion_depth_2_arity_1(self):
        # todo probably should raise some exception in this case....
        fg = FunctorSignatureGenerator(arities={1}, max_recursion_depth=2, random=True)
        functors = list(fg.generate())
        func_non_rec = Functor(name=fg.functor_name, items=[Variable(name=fg.variable_name)])
